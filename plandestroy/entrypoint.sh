...
set +e
OUTPUT=$(sh -c "TF_IN_AUTOMATION=true terraform plan -destroy -detailed-exitcode -input=false $*" 2>&1)
SUCCESS=$?
echo "$OUTPUT"
set -e
...
if [ $SUCCESS -ne 0 ]; then
    OUTPUT=$(wrap "$OUTPUT")
    COMMENT="#### \`terraform plan -destroy\` Failed
$OUTPUT
*Workflow: \`$GITHUB_WORKFLOW\`, Action: \`$GITHUB_ACTION\`*"
    ...
    COMMENT="#### \`terraform plan -destory\` Success
$OUTPUT
*Workflow: \`$GITHUB_WORKFLOW\`, Action: \`$GITHUB_ACTION\`*"
fi
